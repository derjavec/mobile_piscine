...
class LocationService {
  ...
  static Future<String> getCityName(double latitude, double longitude) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['results'].isNotEmpty) {
          ...
          final city = components['city'] ??
              ...
              'Unknown city';
          final region = components['region'] ?? 'Unknown region';
      ...
      return "Error: The service connection is lost, please check your internet connection or try again later";
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return (data['results'] as List).map((city) {
          return {
            "description": "${city['name']}, ${city['region']}, ${city['country']}",
            "lat": city['latitude'],
            "lon": city['longitude'],
          };
        }).toList();
