...
class WeatherTabs extends StatelessWidget {
  ...
  Widget buildMinMaxTemperatureChart(List<DailyWeather> dailyWeather) {
    ...
    double minTemp = dailyWeather.map((w) => w.minTemp).reduce((a, b) => a < b ? a : b);
    double maxTemp = dailyWeather.map((w) => w.maxTemp).reduce((a, b) => a > b ? a : b);
      ...
      LineChartData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (value, meta) {
                ...
                if (index < 0 || index >= dailyWeather.length) return const SizedBox();
                final day = dailyWeather[index].date;
                return Text(
                  "${day.day}/${day.month}",
                  style: const TextStyle(color: Colors.white, fontSize: 12),
                );
        ...
        lineBarsData: [
          LineChartBarData(
            spots: List.generate(
              dailyWeather.length,
              (i) => FlSpot(i.toDouble(), dailyWeather[i].minTemp),
            ),
            isCurved: true,
          ...
          LineChartBarData(
            spots: List.generate(
              dailyWeather.length,
              (i) => FlSpot(i.toDouble(), dailyWeather[i].maxTemp),
            ),
            isCurved: true,
        ...
        minY: minTemp.floorToDouble() - 2,
        maxY: maxTemp.ceilToDouble() + 2,
      ),
    );
  }
  ...
  String getBackgroundGif(String weather) {
    ...
    if (w.contains("cloud") || w.contains("overcast") || w.contains("fog")) {
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      SizedBox(
                        height: 200,
                        child: buildMinMaxTemperatureChart(weather.cast<DailyWeather>()),
                      ),
                      const SizedBox(height: 16),
                      Expanded(
                        child: ListView.builder(
                          itemCount: weather.length,
                          itemBuilder: (context, index) {
                            final w = weather[index] as DailyWeather;
                            return Padding(
                              padding: const EdgeInsets.symmetric(vertical: 5.0),
                              child: Center(
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(width: 10),
                                    Text(
                                      "${w.minTemp}°C / ${w.maxTemp}°C",
                                      style: const TextStyle(color: Colors.white, fontSize: 16),
                                    ),
                                    const SizedBox(width: 10),
                                    Text(
                                      "${w.codes}", // descripción del clima
                                      style: const TextStyle(color: Colors.white70, fontSize: 14),
                                    ),
                                  ],
                                ),
